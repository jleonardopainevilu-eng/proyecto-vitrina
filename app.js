...
const DB_NAME = "proyecto-vitrina-db";
const DB_VERSION = 1;
const STORE_NAME = "app";
const STATE_KEY = "state-v3";
const LEGACY_STORAGE_KEYS = ["project-vitrine-mvp-v1", "proyecto-vitrina-mvp-v1"];
...
function idbPut(key, value) {
  return new Promise((resolve, reject) => {
    ...
    transaction.onabort = () => reject(transaction.error || new Error("IndexedDB canceló la escritura"));
  });
}
...
async function makeLocalStorageSafe(value) {
  if (value instanceof Blob) return blobToDataUrl(value);
  if (Array.isArray(value)) return Promise.all(value.map(makeLocalStorageSafe));
  if (value && typeof value === "object") {
    const entries = await Promise.all(Object.entries(value).map(async ([key, item]) => [key, await makeLocalStorageSafe(item)]));
...
function readLegacyState() {
  for (const key of LEGACY_STORAGE_KEYS) {
    try {
      ...
      console.warn("No se pudo leer un estado anterior", error);
    }
  }
  return null;
}
...
async function initializeStorage() {
  try {
    ...
    state = normalizeState(stored || legacy || getDefaultState());
    if (!stored) await idbPut(STATE_KEY, state);
    updateStorageStatus("IndexedDB · imágenes locales", "ready");
  } catch (error) {
    console.warn("IndexedDB no disponible; se usará respaldo local", error);
    storageMode = "localstorage";
    try {
      const raw = localStorage.getItem(STATE_KEY);
      state = normalizeState(raw ? JSON.parse(raw) : readLegacyState() || getDefaultState());
      updateStorageStatus("Respaldo local", "fallback");
    } catch (localError) {
      console.warn("Tampoco se pudo usar localStorage", localError);
      storageMode = "memory";
      state = getDefaultState();
      updateStorageStatus("Solo esta sesión", "fallback");
    }
  }
}
  ...
  saveQueue = saveQueue.then(async () => {
    try {
      if (storageMode === "indexeddb") {
        ...
        const safeState = await makeLocalStorageSafe(state);
        localStorage.setItem(STATE_KEY, JSON.stringify(safeState));
      }
      return true;
    } catch (error) {
      console.error("Error al guardar", error);
      showToast("No se pudo guardar. El almacenamiento del navegador puede estar lleno.");
      return false;
    }
  });
  return saveQueue;
...
function conceptualProfileUrl() {
  return `https://proyectovitrina.cl/${state.profile.handle}`;
}

function conceptualProductUrl(productId) {
  return `${conceptualProfileUrl()}/producto/${productId}`;
}
...
function renderProfile() {
  ...
  el("platformBranding").textContent = "Creado con Proyecto Vitrina · prototipo funcional";
}
...
async function handleMediaUpload(event, target, maxDimension, quality) {
  ...
  try {
    ...
    state.profile[target] = compressed;
    await persistState();
    renderAll();
    showToast(target === "avatar" ? "Foto de perfil actualizada." : "Portada actualizada.");
  } catch (error) {
    console.error(error);
    showToast("No fue posible procesar esa imagen. En iPhone, prueba exportarla como JPG.");
  } finally {
    event.target.value = "";
  }
}
...
function bindEvents() {
  el("toggleEditorButton").addEventListener("click", () => showEditor("profile"));
