...
function showAuthError(error) {
  ...
  box.textContent = error?.message || "Ocurrió un error. Intenta de nuevo.";
  box.hidden = false;
}
...
async function createVitrinaRow(userId, initialState) {
...
async function initializeStorage() {
  ...
  if (!currentSession) {
    ...
    updateStorageStatus("Sin sesión", "fallback");
    showAuthScreen();
    return;
  }
  ...
  try {
    ...
    if (row) {
      ...
      await createVitrinaRow(currentSession.user.id, state);
    }
    storageMode = "supabase";
...
function bindEvents() {
  el("authSignInButton").addEventListener("click", async (event) => {
    event.preventDefault();
    el("authError").hidden = true;
    try {
      await signInWithEmail(el("authEmail").value.trim(), el("authPassword").value);
      window.location.reload();
    } catch (error) {
      showAuthError(error);
    }
  });
  el("authSignUpButton").addEventListener("click", async (event) => {
    event.preventDefault();
    el("authError").hidden = true;
    try {
      await signUpWithEmail(el("authEmail").value.trim(), el("authPassword").value);
...
async function init() {
  ...
  if (!currentSession && !publicViewHandle) {
    // Sin sesión y sin vitrina pública que ver: se queda en la pantalla de login.
    return;
  }
